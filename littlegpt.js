...
async function cmdHelp(messageId) {
  helpText = `LittleGPT Command List
...
async function handleReply(userInput, sessionId, messageId, eventId) {
  ...
  // Check if the question contains a mention
  const mentionRegex = /@([^ ]+)/;
  const mentionMatch = question.match(mentionRegex);
  
  if (mentionMatch) {
    // Extract the mentioned user's name or ID
    const mentionedUser = mentionMatch[1];
    
    try {
      // Call the correct API endpoint to get information about the mentioned user
      const response = await client.im.chat.getMembers({
        chat_id: sessionId,
      ...
      if (response.code === 0 && response.data.items.length > 0) {
        const mentionedUserInfo = response.data.items[0];
        const mentionedUserName = mentionedUserInfo.name;
        
        // Modify the question to include the mentioned user's name
        question = question.replace(mentionMatch[0], `@${mentionedUserName}`);
        
        // Mention the user by name in the bot's response
        await reply(messageId, `I apologize, but I don't have access to personal information, including ${mentionedUserName}'s details.`);
        return { code: 0 };
      }
    } catch (error) {
      logger('Error getting member information from Lark API', error);
    }
  }
